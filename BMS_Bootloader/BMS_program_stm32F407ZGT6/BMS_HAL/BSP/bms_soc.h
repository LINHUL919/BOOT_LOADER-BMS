 ...
 *    7) EEPROM保存: 每60s检查, 变化≥1% full_cap 才写入(减少擦写)
 */
...
/* ---- EEPROM持久化 (F407移植: Flash→W24C02) ---- */
#define SOC_EEPROM_SAVE_INTERVAL_S  60      /* EEPROM检查保存间隔 s */
#define SOC_EEPROM_BASE_ADDR    0x20        /* W24C02中SOC数据起始地址
                                             * 0x10~0x12被Bootloader占用
                                             * SOC数据存在 0x20~0x33 (20字节) */
#define SOC_EEPROM_DEV_ADDR     0xA0        /* W24C02 I2C写地址 */
...
typedef struct {
    ...
    /* ---- EEPROM管理 ---- */
    uint32_t eeprom_timer_ms;        /* EEPROM保存计时 ms */
...
/**
 * @brief  SOC模块初始化 — 从EEPROM恢复或设为默认值
