...
uint8_t App_bootloader_check_data(void)
{
    ...
    if (app_total_length == total_length)
    {
        ...
        // 修改状态，准备跳转到应用程序
        app_bootloader_state = APP_BOOTLOADER_STATE_JUMP_TO_APP;
        return 0;
    }
    else
    {
...
{
    // 直接调用int_bootloader.c中已经实现的跳转函数
    uint8_t result = Int_bootloader_jump_to_app();
    return result;
}
...
void App_bootlaoder_work(void)
{
    ...
    case APP_BOOTLOADER_STATE_CHECK_DATA:
        ...
        break;
    case APP_BOOTLOADER_STATE_JUMP_TO_APP:
        // 跳转到应用程序
        if (App_bootloader_jump_to_app())
        {
            // 跳转失败，重新启动boot loader
            printf("Jump to app failed, restarting bootloader\r\n");
            NVIC_SystemReset(); // 校验失败，重启系统回到初始状态
        }
