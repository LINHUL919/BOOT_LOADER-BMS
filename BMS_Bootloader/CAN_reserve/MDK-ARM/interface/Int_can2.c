...
void CAN2_Init(void)
{
    ...
    sFilterConfig.FilterBank = 14; //使用第14个过滤器
 ...
 * 这里我主要使用数据帧就行了不用远程帧
 */
void CAN2_SendData(uint16_t id, uint8_t *data, uint8_t length)
{ 
    //等待发送邮箱空闲
 ...
 * @return {*}
 */
void CAN2_ReceiveData(CAN_MessageTypeDef *rec_msg,uint8_t *msg_count)
{
    *msg_count = HAL_CAN_GetRxFifoFillLevel(&hcan2, CAN_RX_FIFO0);
     for (uint8_t i = 0;i < *msg_count;i++ )
     {
        ...
        //获取当前缓存的CAN报文
        HAL_CAN_GetRxMessage(&hcan2, CAN_RX_FIFO0, &(current_msg->RxHeader), current_msg->data);
     }
}
