...
void W25Q128_ReadData(uint8_t block_ID,
                      ...
                      uint16_t size)
                      {
                        ...
                        //计算24位地址：块ID占6位，扇区ID占4位，页ID占4位，页内地址占8位
                        uint32_t address = ((block_ID & 0x0F) << 16) | ((sector_ID & 0x0F) << 12) | ((page_ID & 0x0F) << 8) | (addr & 0xFF);
                        //发送24位地址，分三次发送，每次8位
...
void W25Q128_EraseSector(uint8_t block_ID,uint8_t sector_ID)
{
    ...
    //计算24位地址：块ID占6位，扇区ID占4位，页ID占4位，页内地址占8位，这里页ID和页内地址都为0，因为擦除是以扇区为单位的
    uint32_t address = ((block_ID & 0x0F) << 16) | ((sector_ID & 0x0F) << 12);
    //发送24位地址，分三次发送，每次8位
    W25Q128_WriteByte((address >> 16) & 0xFF); //地址高8位
    W25Q128_WriteByte((address >> 8) & 0xFF);  //地址中8位
    W25Q128_WriteByte(address & 0xFF);         //地址低8位
